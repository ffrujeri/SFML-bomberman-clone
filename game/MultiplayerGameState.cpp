		...
		if (mTickClock.getElapsedTime() > sf::seconds(1.f / 20.f))
		{
			...
			{			
				if (Character * aircraft = mWorld.getCharacter(identifier))
					positionUpdatePacket << identifier << aircraft->getPosition().x << aircraft->getPosition().y << static_cast<sf::Int32>(aircraft->getHitpoints()) << static_cast<sf::Int32>(aircraft->getGuidedBombAmmo());
			}
...
void MultiplayerGameState::handlePacket(sf::Int32 packetType, sf::Packet& packet)
{
	switch (packetType)
	{
		...
		case Server::SpawnSelf:
		{
			...
			Character * aircraft = mWorld.addCharacter(aircraftIdentifier);
		...
		case Server::PlayerDisconnect:
		{
			...
			mWorld.removeCharacterById(aircraftIdentifier);
		...
		case Server::InitialState:
		{
			...
			//Check why the tiles disappear when seting world height/position.
			//mWorld.setWorldHeight(worldHeight);
			//mWorld.setCurrentFieldPosition(currentScroll);
			...
			for (sf::Int32 i = 0; i < aircraftCount; ++i)
			{
				...
				aircraft->setGuidedBombAmmo(missileAmmo);
		...
		case Server::AcceptCoopPartner:
		{
			...
			mWorld.addCharacter(aircraftIdentifier);
		...
		// Player's movement or placeBomb keyboard state changes
		...
		{
			requestStackPush(States::GameSuccess);
		} break;
